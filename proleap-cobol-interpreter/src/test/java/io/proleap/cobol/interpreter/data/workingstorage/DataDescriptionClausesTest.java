...
public class DataDescriptionClausesTest extends TestBase {
	...
	public void test() throws Exception {
		...
		assertEquals(Java11To8.repeat(" ", 10), valueService.getString(getValue("ITEM", programUnit, state)));
		assertEquals(Java11To8.repeat(" ", 5), valueService.getString(getValue("ITEMRED", programUnit, state)));
		assertEquals(BigDecimal.ZERO, valueService.getDecimal(getValue("ITEMINT", programUnit, state)));
		assertEquals(Java11To8.repeat(" ", 10), valueService.getString(getValue("ITEMSTR", programUnit, state)));
		assertEquals(Java11To8.repeat(" ", 10), valueService.getString(getValue("ITEMEXT", programUnit, state)));
		assertEquals(Java11To8.repeat(" ", 10), valueService.getString(getValue("ITEMGLB", programUnit, state)));
		assertEquals(Java11To8.repeat(" ", 10), valueService.getString(getValue("ITEMTPD", programUnit, state)));
		assertEquals(Java11To8.repeat(" ", 10), valueService.getString(getValue("ITEMTHR", programUnit, state)));
